...
public class ProblemAlgo {
     ...
     */
    public static ArrayList<ProblemRequestDTO.ProblemAlgoDTO> getProblemsByTag(String request) {
        try {
            // DB 연결
            ...
            // 쿼리 작성 - 고민 JOIN 쓸까?
            String query = "SELECT * FROM problems WHERE problem_id = (SELECT problem_id FROM proalgo WHERE algo_id = (SELECT algo_id FROM algorithms WHERE algo ="+whichTag+")) ORDER BY solved_people DESC";
            PreparedStatement pstmt = conn.prepareStatement(query);

            ResultSet rs = pstmt.executeQuery();
            ArrayList<ProblemRequestDTO.ProblemAlgoDTO> result = new ArrayList<>();

            // 결과 처리
            while (rs.next()) {
                // 튜플에서 problemId, title, link, solvedPeople, tier 추출
                ...
                result.add(new ProblemRequestDTO.ProblemAlgoDTO(problemId, title, link, solvedPeople, tier));
            }
